...
void A_output(struct msg message)
{
  ...
  if ( A_nextseqnum < base + WINDOWSIZE && windowcount < WINDOWSIZE ) {
    ...
    /*The sender should start timer after sending a packet*/
...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet)) {
    ...
    if (packet.acknum>=base) {    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
      ...
      /*The sender should stop the timer after a correct ack*/
      stoptimer(A);
...
void A_timerinterrupt(void)
{
  ...
  //Resend from the begining
  for(i=0;i<windowcount;i++){
      if (TRACE > 0)
          printf ("---A: resending packet %d\n", (buffer[temp]).seqnum);

      tolayer3(A,buffer[temp]);

      //start timer at the first time
      if(temp==windowfirst){
          starttimer(A,RTT);
      }

      packets_resent++;
      temp = (temp + 1) % WINDOWSIZE;
    }
}       
...
void B_input(struct pkt packet)
{
  ...
  if  ( (!IsCorrupted(packet))  && (packet.seqnum == expectedseqnum) ) {
    ...
    /*The acknumber should be the oppsite of the packet number*/
...
void B_init(void)
{
  /*Change the expectedseqnum to 0 when initiate*/
  expectedseqnum = 0;
  B_nextseqnum = 1;
}
